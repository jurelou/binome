...
class SingleCollector(BaseScan):
    ...
    def launch(self):
        def check_collector_exists(collector_name):
            from opulence.engine.controllers.agents import available_agents # pragma: nocover
        ...
        agent_tasks.scan(self.config.external_id, self.config.collector_name, self.config.facts)
